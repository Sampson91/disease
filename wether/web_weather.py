            ...
            for k in range(1,months+1):       #60个月 2011-2016
                ...
                for d in range(1,days+1):    #每个月多少天
                    ...
                    try:
                        ...
                        weather_day   = pinyin.get(weather_day, format='strip', delimiter="")
                        weather_night = wether.split("/")[1]
                        weather_night = pinyin.get(weather_night, format='strip', delimiter="")
                        ...
                        wind_day1   = pinyin.get(wind_day1, format='strip', delimiter="")
                        wind_day2   = wind_day.split(" ")[1]
                        wind_day2   = pinyin.get(wind_day2, format='strip', delimiter="")
                        wind_night  = wind.split("/")[1]
                        wind_night1 = wind_night.split(" ")[0]
                        wind_night1 = pinyin.get(wind_night1, format='strip', delimiter="")
                        wind_night2 = wind_night.split(" ")[1]
                        wind_night2 = pinyin.get(wind_night2, format='strip', delimiter="")
                    except:
                        continue
                ...
                cur = conn.cursor()
                cur.executemany('insert into weather2 values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', values)
                conn.commit()
                cur.close()
                conn.close()
if __name__ == '__main__':
    ...
    conn = MySQLdb.connect(host='localhost', user=user1, passwd=password1, db=mysqlDB, port=3306,charset="utf8")
    cur = conn.cursor()
    cur.execute('create table weather2 (city_first varchar(300),shi varchar(300),time datetime,weather_day varchar(300),weather_night varchar(300),tem_day int,tem_night int,wind_day1 varchar(300),wind_day2 varchar(300),wind_night1 varchar(300),wind_night2 varchar(300))')
    conn.commit()
    cur.close()
    conn.close()
